...
module.exports = function (config, logger, cb) {
    ...
    async.eachSeries(config.tests, function (item, cb) {
        ...
        logger.info("Screenshots from {cyan:%s} saved in: {yellow:%s", hash1, path.join(config.outDir, hash1));
        logger.info("Screenshots from {cyan:%s} saved in: {yellow:%s", hash2, path.join(config.outDir, hash2));
        cb(null, {
            config: config
        });
    });
